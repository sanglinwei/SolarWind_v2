...
if __name__ == '__main__':
    ...
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=None)
    surf = ax.plot_surface(X, Y, cap_sigma_rob_pd.iloc[:, :], cmap=cm.coolwarm, linewidth=0, antialiased=False)
    cset = ax.contourf(X, Y, cap_sigma_rob_pd.to_numpy(), zdir='z', offset=np.min(cap_sigma_rob_pd.to_numpy()), cmap=cm.coolwarm)
    ax.set_xlabel('抽蓄占比', fontproperties=font, rotation=-15)
    ax.set_ylabel('不确定性', fontproperties=font, rotation=50)
    ax.set_zlabel('新能源消纳容量/MW', fontproperties=font)
    plt.margins(x=0)
