...
if __name__ == '__main__':
    ...
    for sigma in np.linspace(0, 0.2, 5+1):
        ...
        cap_sigma_rob[sigma] = cap_rob
        thermal_sigma_rob[sigma] = thermal_rob
        # 求随机结果（平均场景）
        cap_mean = np.mean(cap_cat, axis=2)
        thermal_mean = np.mean(thermal_cat, axis=2)
        cap_sigma_sto[sigma] = cap_mean
        thermal_sigma_sto[sigma] = thermal_mean


    def dic_np2list(dic_np):
        return [dic_np[i].reshape(-1).tolist() for i in np.linspace(0, 0.2, 5+1)]
    thermal_sigma_sto_pd = pd.DataFrame(dic_np2list(thermal_sigma_sto))
    ...
    surf = ax.plot_surface(X, Y, cap_sigma_sto_pd.iloc[:, :], cmap=cm.coolwarm, linewidth=0, antialiased=False)
    cset = ax.contourf(X, Y, cap_sigma_sto_pd.to_numpy(), zdir='z', offset=np.min(cap_sigma_sto_pd.to_numpy()), cmap=cm.coolwarm)
    ax.set_xlabel('抽蓄占比', fontproperties=font, rotation=-15)
    ax.set_ylabel('不确定性', fontproperties=font, rotation=50)
    ax.set_zlabel('新能源消纳容量/MW', fontproperties=font)
    plt.margins(x=0)
    plt.margins(y=0)
    plt.grid()
    plt.colorbar(surf, ax=[ax], location='left', shrink=0.7, aspect=10, pad=0)
    plt.savefig('./figs/消纳容量随机优化m3.png', dpi=900, transparent=True, pad_inches=0)
    plt.show()
    ...
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=None)
    surf = ax.plot_surface(X, Y, cap_sigma_rob_pd.iloc[:, 1:], cmap=cm.coolwarm, linewidth=0, antialiased=False)
    cset = ax.contourf(X, Y, cap_sigma_rob_pd.to_numpy()[:, 1:], zdir='z', offset=np.min(cap_sigma_rob_pd.to_numpy()[:, 1:]), cmap=cm.coolwarm)
    ax.set_xlabel('抽蓄占比', fontproperties=font, rotation=-15)
    ax.set_ylabel('不确定性', fontproperties=font, rotation=50)
    ax.set_zlabel('新能源消纳容量/MW', fontproperties=font)
    plt.margins(x=0)
    ...
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=None)
    surf = ax.plot_surface(X, Y, cap_sigma_rob_pd.iloc[:, :], cmap=cm.Blues, linewidth=0, antialiased=False)
    surf = ax.plot_surface(X, Y, cap_sigma_sto_pd.iloc[:, :], cmap=cm.Greens, linewidth=0, antialiased=False)
    ax.set_xlabel('抽蓄占比', fontproperties=font, rotation=-15)
    ax.set_ylabel('不确定性', fontproperties=font, rotation=50)
    ax.set_zlabel('新能源消纳容量/MW', fontproperties=font)
    plt.margins(x=0)
    ...
    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=None)
    surf1 = ax.plot_surface(X, Y, thermal_sigma_sto_pd.iloc[:, :], cmap=cm.coolwarm, linewidth=0, antialiased=False)
    surf2 = ax.plot_surface(X, Y, thermal_sigma_rob_pd.iloc[:, :], cmap=cm.coolwarm, linewidth=0, antialiased=False)
    ax.set_xlabel('抽蓄占比', fontproperties=font, rotation=-15)
    ...
    plt.margins(y=0)
    plt.grid()
    plt.colorbar(surf1, ax=[ax], location='left', shrink=0.7, aspect=10, pad=0)
    plt.savefig('./figs/火电调节能力m3.png', dpi=900, transparent=True, pad_inches=0)
    plt.show()
